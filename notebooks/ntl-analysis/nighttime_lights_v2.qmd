...
#### MAIN PARAMS
proj_dir     <- "~/Dropbox/World Bank/Side Work/Lebanon Economic Monitor v2"
iso3_code    <- "LBN"
iso2_code    <- "LB"
pc_base_year <- 2019
...
ntl_days <- seq.Date(ymd("2019-01-01"), to = Sys.Date(), by = "day") %>%
  as.character()
  ...
  for(time_type in c("annual",
    ...
    for(time_i in time_vec){
      ...
      if(!file.exists(OUT_FILE)){
        ...
        if(unit == "adm1"){
          roi_sf <- st_read(file.path(data_dir, "boundaries", "lbn_adm_cdr_20200810",
                                      "lbn_admbnda_adm1_cdr_20200810.shp"))
        }
        ...
        if(unit == "adm3"){
          roi_sf <- st_read(file.path(data_dir, "boundaries", "lbn_adm_cdr_20200810",
                                      "lbn_admbnda_adm3_cdr_20200810.shp"))
        }

        if(unit == "adm4"){
          roi_sf <- st_read(file.path(data_dir, "boundaries", "lbn_beirut_adm4_mapaction_pcoded",
                                      "lbn_beirut_adm4_MapAction_Pcoded.shp"))
        }
...
```{r}
### ADM 0
ntl_df <- readRDS(file.path(ntl_bm_dir, "aggregated", paste0("ntl_adm0_daily.Rds")))

ntl_df <- ntl_df %>%
  mutate(date = date %>% ymd() %>% floor_date(unit = "months")) %>%
  group_by(GID_0, COUNTRY, date) %>%
  dplyr::summarise(ntl_sum  = mean(ntl_sum, na.rm = T),
                   ntl_mean = mean(ntl_mean, na.rm = T)) %>%
  ungroup()

saveRDS(ntl_df,   file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm0", "_", "monthly_from_daily", ".Rds")))
write_csv(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm0", "_", "monthly_from_daily", ".csv")))
write_dta(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm0", "_", "monthly_from_daily", ".dta")))

### ADM 1
ntl_df <- readRDS(file.path(ntl_bm_dir, "aggregated", paste0("ntl_adm1_daily.Rds")))
...
saveRDS(ntl_df,   file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm1", "_", "monthly_from_daily", ".Rds")))
write_csv(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm1", "_", "monthly_from_daily", ".csv")))
write_dta(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm1", "_", "monthly_from_daily", ".dta")))


### ADM 2
ntl_df <- readRDS(file.path(ntl_bm_dir, "aggregated", paste0("ntl_adm2_daily.Rds")))
...
saveRDS(ntl_df,   file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm2", "_", "monthly_from_daily", ".Rds")))
write_csv(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm2", "_", "monthly_from_daily", ".csv")))
write_dta(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm2", "_", "monthly_from_daily", ".dta")))

ntl_ml_df <- ntl_df[ntl_df$admin1Name %in% "Mount Lebanon",]
saveRDS(ntl_ml_df,   file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm2", "_", "monthly_from_daily_mt_lebanon", ".Rds")))
write_csv(ntl_ml_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm2", "_", "monthly_from_daily_mt_lebanon", ".csv")))
write_dta(ntl_ml_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm2", "_", "monthly_from_daily_mt_lebanon", ".dta")))

### ADM 3
ntl_df <- readRDS(file.path(ntl_bm_dir, "aggregated", paste0("ntl_adm3_daily.Rds")))
...
saveRDS(ntl_df,   file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm3", "_", "monthly_from_daily", ".Rds")))
write_csv(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm3", "_", "monthly_from_daily", ".csv")))
write_dta(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm3", "_", "monthly_from_daily", ".dta")))

ntl_ml_df <- ntl_df[ntl_df$admin1Name %in% "Mount Lebanon",]
saveRDS(ntl_ml_df,   file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm3", "_", "monthly_from_daily_mt_lebanon", ".Rds")))
write_csv(ntl_ml_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm3", "_", "monthly_from_daily_mt_lebanon", ".csv")))
write_dta(ntl_ml_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm3", "_", "monthly_from_daily_mt_lebanon", ".dta")))

### ADM 4
ntl_df <- readRDS(file.path(ntl_bm_dir, "aggregated", paste0("ntl_adm4_daily.Rds")))
...
saveRDS(ntl_df,   file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm4", "_", "monthly_from_daily", ".Rds")))
write_csv(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm4", "_", "monthly_from_daily", ".csv")))
write_dta(ntl_df, file.path(ntl_bm_dir, "aggregated", paste0("ntl_", "adm4", "_", "monthly_from_daily", ".dta")))
```
...
ntl_df %>%
  mutate(date = date %>% ymd()) %>%
  dplyr::select(date, COUNTRY, ntl_sum) %>%
...
ntl_df %>%
  mutate(date = date %>% ymd()) %>%
  dplyr::select(date, NAME_1, ntl_sum) %>%
...
ntl_df %>%
  mutate(date = date %>% ymd()) %>%
  dplyr::select(date, NAME_2, ntl_sum) %>%
...
ntl_df %>%
  ...
  dplyr::select(date, city_name, geoname_id, ntl_sum) %>%
...
## Figures
```{r}
month_df <- readRDS(file.path(ntl_bm_dir, "aggregated", "ntl_adm0_monthly.Rds"))
...
p <- month_df %>%
  ...
  geom_col(aes(x = date,
               y = ntl_sum),
           fill = wbg_color_dark) +
  labs(title = "Monthly Nighttime Lights: ADM1",
       y = "NTL Radiance",
       x = NULL) +
  theme_classic2() +
  theme(strip.background = element_blank(),
...
ggsave(p, filename = "~/Documents/Github/lebanon-economic-monitor/notebooks/ntl-analysis/figures/monthly_trends_adm0.png",
       height = 3, width = 5)

#### ADM1, Month
month_df <- readRDS(file.path(ntl_bm_dir, "aggregated", "ntl_adm1_monthly.Rds"))
...
ggsave(p, filename = "~/Documents/Github/lebanon-economic-monitor/notebooks/ntl-analysis/figures/monthly_trends_adm1.png",
       height = 3.5, width = 5)

#### ADM0, Daily
month_df <- readRDS(file.path(ntl_bm_dir, "aggregated", "ntl_adm0_daily.Rds"))
...
p <- month_df %>%
  ...
  geom_col(aes(x = date,
               y = ntl_sum),
           fill = wbg_color_dark) +
  labs(title = "Daily Nighttime Lights: ADM1",
       y = "NTL Radiance",
       x = NULL) +
  theme_classic2() +
  theme(strip.background = element_blank(),
...
ggsave(p, filename = "~/Documents/Github/lebanon-economic-monitor/notebooks/ntl-analysis/figures/daily_trends_adm0.png",
       height = 3, width = 5)

#### ADM1, Month
month_df <- readRDS(file.path(ntl_bm_dir, "aggregated", "ntl_adm1_daily.Rds"))
...
ggsave(p, filename = "~/Documents/Github/lebanon-economic-monitor/notebooks/ntl-analysis/figures/daily_trends_adm1.png",
       height = 3.5, width = 5)
